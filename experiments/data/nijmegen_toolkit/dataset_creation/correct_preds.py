...
def start_interactive_annotator(args):
    annotation_dir = os.path.join(args.root_dir, "corrected_predictions")
    os.makedirs(annotation_dir, exist_ok=True)
    embedding_paths = os.path.join(args.root_dir, "embeddings")
    image_paths = natsorted(glob(os.path.join(args.root_dir, "annotated_images", "*.tiff")))
    pred_dir = os.path.join(args.root_dir, "filtered_predictions_v5")
    correction_version = args.correction

    for image_path in image_paths:
        img_name = os.path.basename(image_path).strip(".tiff")
        ...
        if os.path.exists(annotation_path):
            ...
            if os.path.exists(previous_version):
                ...
                grid_label = f"{img_name}-version_{correction_version - 1}"
                ...
                if len(prelim_annotations) > 0:
                    grid_label = img_name + "-" + "".join(prelim_annotations[0].strip("_prelim.tiff").split("_")[-1])
                    segmentation_result = imageio.imread(prelim_annotations[0])
                    print("Successfully loaded preliminary annotation")
                else:
                    pred_path = os.path.join(pred_dir, os.path.basename(image_path).replace(".tiff", "_pred.tiff"))
                    segmentation_result = imageio.imread(pred_path)
                    print("Loaded untouched PathoSAM pred")
                    grid_label = img_name + "-fresh"

        embedding_path = os.path.join(embedding_paths, os.path.basename(image_path.strip(".tiff")))
        ...
        if args.grid_path is not None:
            ...
            shapes = []
            for __, group in df.groupby("index"):
        ...
        def save_prelim_annotation(viewer: "napari.Viewer", note: str = ""):
            ...
            prelim_path = annotation_path.replace(f"_label_{correction_version}.tiff",
                                                  f"_{note}_prelim_{correction_version}.tiff")
            imageio.imwrite(annotation_path.replace(f"_label_{correction_version}.tiff",
                                                    f"_{note}_prelim_{correction_version}.tiff"), ann, plugin="tifffile", compression="zlib")
        ...
        def save_annotation(viewer: "napari.Viewer"):
            ...
            imageio.imwrite(annotation_path, definite_ann, plugin="tifffile", compression="zlib")
            prelim_annotations = glob(os.path.join(annotation_dir, f"*{img_name}_*_prelim*"))
            for prelim_annotation in prelim_annotations:
                os.remove(prelim_annotation)
        ...
        napari.run()
    print(f"Annotation round {correction_version} completed!")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--root_dir", default="/mnt/ceph-hdd/cold/nim00020/hannibal_data/ignite_nijmegen")
