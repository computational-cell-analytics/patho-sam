...
def create_val_split(
    ...
    dst_paths = {
        f"{split}_labels": Path(path) / custom_name / f"{split}_labels"
        for split in splits
    }
    dst_paths.update({
        f"{split}_images": Path(path) / custom_name / f"{split}_images"
        for split in splits
    })
...
def preprocess_datasets(eval_path, data_path, model_names=['pannuke_sam', 'vanilla_sam'], prompt=False):
    ...
    for dataset in datasets:
        ...
        create_val_split(
            ...
            test_percentage=0.95, custom_name='eval_split', random_seed=42
        )
