...
def run_boxes_inference(model_dir, input_dir):
    for model in MODEL_NAMES:
        ...
        for dataset in [
            ...
            output_path = os.path.join(model_dir, model, 'inference', dataset, 'boxes')
            os.makedirs(output_path, exist_ok=True)
            if os.path.exists(os.path.join(output_path, 'results', 'iterative_prompting_without_mask', 'iterative_prompts_start_box.csv')):
            ...
            command = ['python3', '/user/titus.griebel/u12649/patho-sam/experiments/histopathology/evaluate_iterative_prompting.py'] + args
